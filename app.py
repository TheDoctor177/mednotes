...
def extract_pdf_text_and_images(pdf_path: str, out_dir: Path) -> Tuple[str, List[str]]:
    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        for img_index, img in enumerate(page.get_images(full=True), start=1):
            ...
            image_path = out_dir / f"pdf_page_{page_num}_img_{img_index}.{image_ext}"
            with open(image_path, "wb") as f:
                f.write(image_bytes)
            image_paths.append(str(image_path))

    doc.close()
    return "\n".join(texts).strip(), image_paths


def extract_pptx_text_and_images(pptx_path: str, out_dir: Path) -> Tuple[str, List[str]]:
    ...
    for slide_index, slide in enumerate(prs.slides, start=1):
        ...
        for shape in slide.shapes:
            ...
            if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                ...
                image_path = out_dir / f"slide_{slide_index}_{shape.shape_id}.{ext}"
                with open(image_path, "wb") as f:
                    f.write(image.blob)
                image_paths.append(str(image_path))
        if slide_text:
    ...
    return "\n".join(slide_texts).strip(), image_paths
...
def build_prompt(source_text: str, card_style: str, include_input_figures: bool, create_diagrams: bool) -> str:
    ...
    if include_input_figures:
        figure_instructions.append(
            "If source figures/images are available, provide a short list of 1-5 candidate figure captions or labels that would be useful to display next to the notes."
        )
    if create_diagrams:
        figure_instructions.append(
...
{{
  ...
  "suggested_input_figure_keywords": ["..."],
...
def process_uploaded_file(uploaded_file, run_dir: Path, whisper_model: str) -> Dict[str, Any]:
    ...
    if suffix in [".mp3", ".wav", ".m4a", ".mp4", ".mov", ".mkv"]:
        ...
        source_text, image_paths = extract_pdf_text_and_images(str(save_path), run_dir)
    elif suffix == ".pptx":
        source_text, image_paths = extract_pptx_text_and_images(str(save_path), run_dir)
    else:
...
st.write(
    "Upload audio, video, PDF, or PPTX, or paste a YouTube link. Generate notes, Anki flashcards, and optional figures/diagrams."
)
...
if st.button("Generate MedNotes"):
    if not youtube_url and uploaded_file is None:
        st.error("Add either a YouTube link or an uploaded file.")
        st.stop()

    source_label = youtube_url if youtube_url else uploaded_file.name
    ...
    try:
        ...
        source_text = processed["source_text"].strip()
        ...
        with tab1:
            ...
            st.text_area("Notes", notes_text, height=350)
        ...
        with tab3:
            if include_input_figures:
                ...
                picked = pick_images_to_display(image_paths, max_images=max_images)
                if picked:
        ...
        with tab4:
            raw_label = "Transcript" if transcript else "Extracted source text"
            if show_transcript:
                st.text_area(raw_label, source_text, height=350)
